...
public class ColorPaletteControllerAraba12 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
{
    public static ColorPaletteControllerAraba12 Current;
    ...
    {
        PlayerPrefs.SetString("SavedBodyAraba12Color", ColorToHexBody(color));
    }

    void SaveColorRim(Color color)
    {
        PlayerPrefs.SetString("SavedRimAraba12Color", ColorToHexRim(color));
    }
    ...
    public void UpdateColor()
    {
        ...
        if (pickedColorImage)
        {
            pickedColorImage.color = color;
            SelectedColor = color;
            if (VehicleColorAraba12.Current.chechFonkBody == true)
            {
                MateyelGuncellemeBody();
                SaveColorBody(kalicibody.color);
                VehicleColorAraba12.Current.chechFonkBody = false;
            }
            else if (VehicleColorAraba12.Current.checkFonkRim == true)
            {
                MateyelGuncellemeRim();
                SaveColorRim(kalicirim.color);
                VehicleColorAraba12.Current.checkFonkRim = false;
            }
            else if (VehicleColorAraba12.Current.chechFonkBody == false && VehicleColorAraba12.Current.checkFonk2 == true || VehicleColorAraba12.Current.checkFonkRim == false && VehicleColorAraba12.Current.checkFonk2 == true)
            {
                MateyelEsitlemeBody();
                MateyelEsitlemeRim();
            }
