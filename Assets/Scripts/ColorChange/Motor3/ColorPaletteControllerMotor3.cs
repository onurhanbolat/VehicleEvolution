...
public class ColorPaletteControllerMotor3 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
{
    public static ColorPaletteControllerMotor3 Current;
    ...
    {
        PlayerPrefs.SetString("SavedMotor3BodyColor", ColorToHexBody(color));
    }

    void SaveColorRim(Color color)
    {
        PlayerPrefs.SetString("SavedMotor3RimColor", ColorToHexRim(color));
    }
    ...
    public void UpdateColor()
    {
        ...
        if (pickedColorImage)
        {
            pickedColorImage.color = color;
            SelectedColor = color;
            if (VehicleColorMotor3.Current.chechFonkBody == true)
            {
                MateyelGuncellemeBody();
                SaveColorBody(kalicibody.color);
                VehicleColorMotor3.Current.chechFonkBody = false;
            }
            else if (VehicleColorMotor3.Current.checkFonkRim == true)
            {
                MateyelGuncellemeRim();
                SaveColorRim(kalicirim.color);
                VehicleColorMotor3.Current.checkFonkRim = false;
            }
            else if (VehicleColorMotor3.Current.chechFonkBody == false && VehicleColorMotor3.Current.checkFonk2 == true || VehicleColorMotor3.Current.checkFonkRim == false && VehicleColorMotor3.Current.checkFonk2 == true)
            {
                MateyelEsitlemeBody();
                MateyelEsitlemeRim();
            }
