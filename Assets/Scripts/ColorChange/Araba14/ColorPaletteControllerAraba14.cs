...
public class ColorPaletteControllerAraba14 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
{
    public static ColorPaletteControllerAraba14 Current;
    ...
    {
        PlayerPrefs.SetString("SavedBodyAraba14Color", ColorToHexBody(color));
    }

    void SaveColorRim(Color color)
    {
        PlayerPrefs.SetString("SavedRimAraba14Color", ColorToHexRim(color));
    }
    ...
    public void UpdateColor()
    {
        ...
        if (pickedColorImage)
        {
            pickedColorImage.color = color;
            SelectedColor = color;
            if (VehicleColorAraba14.Current.chechFonkBody == true)
            {
                MateyelGuncellemeBody();
                SaveColorBody(kalicibody.color);
                VehicleColorAraba14.Current.chechFonkBody = false;
            }
            else if (VehicleColorAraba14.Current.checkFonkRim == true)
            {
                MateyelGuncellemeRim();
                SaveColorRim(kalicirim.color);
                VehicleColorAraba14.Current.checkFonkRim = false;
            }
            else if (VehicleColorAraba14.Current.chechFonkBody == false && VehicleColorAraba14.Current.checkFonk2 == true || VehicleColorAraba14.Current.checkFonkRim == false && VehicleColorAraba14.Current.checkFonk2 == true)
            {
                MateyelEsitlemeBody();
                MateyelEsitlemeRim();
            }
