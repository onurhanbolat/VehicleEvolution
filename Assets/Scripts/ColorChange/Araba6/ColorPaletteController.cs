...
{
    public static ColorPaletteController Current;
    ...
    {
        PlayerPrefs.SetString("SavedBodyColor", ColorToHexBody(color));
    }

    void SaveColorRim(Color color)
    {
        PlayerPrefs.SetString("SavedRimColor", ColorToHexRim(color));
    }
    ...
    public void UpdateColor()
    {
        ...
        if (pickedColorImage)
        {
            pickedColorImage.color = color;
            SelectedColor = color;
            if (VehicleColor.Current.chechFonkBody == true)
            {
                MateyelGuncellemeBody();
                SaveColorBody(kalicibody.color);
                VehicleColor.Current.chechFonkBody = false;
            }
            else if (VehicleColor.Current.checkFonkRim == true)
            {
                MateyelGuncellemeRim();
                SaveColorRim(kalicirim.color);
                VehicleColor.Current.checkFonkRim = false;
            }
            else if (VehicleColor.Current.chechFonkBody == false && VehicleColor.Current.checkFonk2 == true || VehicleColor.Current.checkFonkRim == false && VehicleColor.Current.checkFonk2 == true)
            {
                MateyelEsitlemeBody();
                MateyelEsitlemeRim();
            }
