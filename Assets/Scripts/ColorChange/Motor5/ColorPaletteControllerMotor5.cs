...
public class ColorPaletteControllerMotor5 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
{
    public static ColorPaletteControllerMotor5 Current;
    ...
    {
        PlayerPrefs.SetString("SavedMotor5BodyColor", ColorToHexBody(color));
    }

    void SaveColorRim(Color color)
    {
        PlayerPrefs.SetString("SavedMotor5RimColor", ColorToHexRim(color));
    }
    ...
    public void UpdateColor()
    {
        ...
        if (pickedColorImage)
        {
            pickedColorImage.color = color;
            SelectedColor = color;
            if (VehicleColorMotor5.Current.chechFonkBody == true)
            {
                MateyelGuncellemeBody();
                SaveColorBody(kalicibody.color);
                VehicleColorMotor5.Current.chechFonkBody = false;
            }
            else if (VehicleColorMotor5.Current.checkFonkRim == true)
            {
                MateyelGuncellemeRim();
                SaveColorRim(kalicirim.color);
                VehicleColorMotor5.Current.checkFonkRim = false;
            }
            else if (VehicleColorMotor5.Current.chechFonkBody == false && VehicleColorMotor5.Current.checkFonk2 == true || VehicleColorMotor5.Current.checkFonkRim == false && VehicleColorMotor5.Current.checkFonk2 == true)
            {
                MateyelEsitlemeBody();
                MateyelEsitlemeRim();
            }
