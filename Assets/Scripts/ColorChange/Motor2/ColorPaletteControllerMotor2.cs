...
public class ColorPaletteControllerMotor2 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
{
    public static ColorPaletteControllerMotor2 Current;
    ...
    {
        PlayerPrefs.SetString("SavedMotor2BodyColor", ColorToHexBody(color));
    }

    void SaveColorRim(Color color)
    {
        PlayerPrefs.SetString("SavedMotor2RimColor", ColorToHexRim(color));
    }
    ...
    public void UpdateColor()
    {
        ...
        if (pickedColorImage)
        {
            pickedColorImage.color = color;
            SelectedColor = color;
            if (VehicleColorMotor2.Current.chechFonkBody == true)
            {
                MateyelGuncellemeBody();
                SaveColorBody(kalicibody.color);
                VehicleColorMotor2.Current.chechFonkBody = false;
            }
            else if (VehicleColorMotor2.Current.checkFonkRim == true)
            {
                MateyelGuncellemeRim();
                SaveColorRim(kalicirim.color);
                VehicleColorMotor2.Current.checkFonkRim = false;
            }
            else if (VehicleColorMotor2.Current.chechFonkBody == false && VehicleColorMotor2.Current.checkFonk2 == true || VehicleColorMotor2.Current.checkFonkRim == false && VehicleColorMotor2.Current.checkFonk2 == true)
            {
                MateyelEsitlemeBody();
                MateyelEsitlemeRim();
            }
