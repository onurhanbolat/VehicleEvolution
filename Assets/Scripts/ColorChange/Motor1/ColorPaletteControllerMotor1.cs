...
public class ColorPaletteControllerMotor1 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IInitializePotentialDragHandler
{
    public static ColorPaletteControllerMotor1 Current;
    ...
    {
        PlayerPrefs.SetString("SavedMotor1BodyColor", ColorToHexBody(color));
    }

    void SaveColorRim(Color color)
    {
        PlayerPrefs.SetString("SavedMotor1RimColor", ColorToHexRim(color));
    }
    ...
    public void UpdateColor()
    {
        ...
        if (pickedColorImage)
        {
            pickedColorImage.color = color;
            SelectedColor = color;
            if (VehicleColorMotor1.Current.chechFonkBody == true)
            {
                MateyelGuncellemeBody();
                SaveColorBody(kalicibody.color);
                VehicleColorMotor1.Current.chechFonkBody = false;
            }
            else if (VehicleColorMotor1.Current.checkFonkRim == true)
            {
                MateyelGuncellemeRim();
                SaveColorRim(kalicirim.color);
                VehicleColorMotor1.Current.checkFonkRim = false;
            }
            else if (VehicleColorMotor1.Current.chechFonkBody == false && VehicleColorMotor1.Current.checkFonk2 == true || VehicleColorMotor1.Current.checkFonkRim == false && VehicleColorMotor1.Current.checkFonk2 == true)
            {
                MateyelEsitlemeBody();
                MateyelEsitlemeRim();
            }
