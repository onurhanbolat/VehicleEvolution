...
public class MotorCont : MonoBehaviour
{

    public static MotorCont Current;
    ...
    public Transform on, car, direksiyon;
    public GameObject shield, shieldkirmizi, karsitoksayac, EffectSoundOnButton, EffectSoundOffButton, infopanel, gizliinfo, MusicSoundOffButton, MusicSoundOnButton;
    ...
    void Update()
    {
        ...
        on.localRotation = Quaternion.Euler(_rotVec);
        ...
        if (carSpeed < 12)
        {
            GameObject.FindWithTag("MainCamera").GetComponent<CameraControl>().offset.z = -3.5f;
        }
        if (carSpeed >= 12)
        {
            GameObject.FindWithTag("MainCamera").GetComponent<CameraControl>().offset.z = -3f;
        }
        if (carSpeed >= 15)
        {
            GameObject.FindWithTag("MainCamera").GetComponent<CameraControl>().offset.z = -2;
        }
        if (carSpeed >= 19)
        {
            GameObject.FindWithTag("MainCamera").GetComponent<CameraControl>().offset.z = -1f;
        }
        if (carSpeed >= 23)
        {
            GameObject.FindWithTag("MainCamera").GetComponent<CameraControl>().offset.z = 0.7f;
        }
        if (carSpeed >= 27)
        {
        ...
        if (carSpeed >= 31)
        {
            GameObject.FindWithTag("MainCamera").GetComponent<CameraControl>().offset.z = 5;
        }
        if(carSpeed >= 40)
        {
            GameObject.FindWithTag("MainCamera").GetComponent<CameraControl>().offset.z = -2;
